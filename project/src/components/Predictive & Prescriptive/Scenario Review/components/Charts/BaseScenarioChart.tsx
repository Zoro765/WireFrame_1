...
interface BaseScenarioChartProps {
  rawData: { name: string; total: number; perUnit: number }[];
  viewMode: "perUnit" | "total";
}

const BaseScenarioChart: React.FC<BaseScenarioChartProps> = ({ rawData, viewMode }) => {
  let cumulative = 0;
  ...
  rawData.forEach((item) => {
    ...
    // Correct color mapping logic (Blue, Green, Red)
  ...
  const layout = {
    title: `Base Scenario (${viewMode})`,
    height: 650,
    width: 1300, // Ensure same width as Chart.js version
    ...
    yaxis: { visible: false },
    showlegend: false,
    bargap: 0.1, // Maintain similar spacing as Chart.js
  };
...
export default BaseScenarioChart;
